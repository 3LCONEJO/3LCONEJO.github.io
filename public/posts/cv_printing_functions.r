...
\n\n\n"
  ...
  for(i in 1:nrow(section_data)){
    for(col in c('title', 'description_bullets')){
      ...
      # strip_res$text <- stringr::str_replace(strip_res$text, "_","<b>") # chenged to italics instead of bold
      # strip_res$text <- stringr::str_replace(strip_res$text, "_","</b>") # fix later
      cv <- strip_res$cv
    }
  }
